...
The performance of FAANG stocks - **Meta (META)**, **Amazon (AMZN)**, **Apple (AAPL)**, **Netflix (NFLX)**, and **Google (GOOGL)**, has become a cornerstone of global financial markets, reflecting broader economic trends and investor sentiment. These companies represent diverse sectors, from e-commerce and technology to entertainment and digital advertising, making their stock price movements critical indicators of market health. As the world navigates the post-pandemic recovery and the uncertainties surrounding the 2024 United States Presidential Election, understanding and forecasting FAANG stock prices is both timely and consequential for investors and policymakers alike.

This paper examines FAANG stock price trends using historical data spanning 2020 to 2024, sourced from Yahoo Finance [@yahoofinance], to develop predictive models for November and December 2024. Utilizing XGBoost regression, the analysis leverages engineered features, including lagged prices, moving averages, and volatility measures, to predict adjusted closing prices for these stocks during the study period. By capturing trends and identifying potential risks and opportunities, these models provide actionable insights for short- and long-term investment strategies. 
...
The implications of these findings extend beyond price predictions, offering a framework to evaluate market behavior in response to external factors such as elections and economic conditions. By contextualizing these trends within the broader market environment, this analysis contributes to a deeper understanding of market dynamics during critical periods. The remainder of this paper is organized as follows: @sec-data describes the data sources, variables, and preprocessing steps; @sec-model outlines the modeling approach and feature engineering strategies; @sec-results presents the results and discusses trends in predicted stock prices; and @sec-discussion addresses the implications and limitations of the findings, with additional model diagnostics detailed in the [Appendix -@sec-model-details].
...
#| fig-cap: "Distribution of Adjusted Closing Prices for FAANG Stocks (January 2020–November 2024). This visualization presents histograms of the adjusted closing prices for Apple (AAPL), Amazon (AMZN), Google (GOOGL), Meta (META), and Netflix (NFLX). Each panel illustrates the frequency of daily closing prices within specific price ranges, highlighting the variability in stock performance across the FAANG group during the observed period."
#| warning: false
...
```
@fig-outcome-var depict the distribution of adjusted closing prices for FAANG stocks over the period from January 2020 to November 2024. AAPL, AMZN, and GOOGL exhibit relatively compact distributions, with prices concentrated between $80–$200 for GOOGL and AMZN and $100–$250 for AAPL. META and NFLX show broader price ranges, with META peaking between $300–$400 and NFLX exhibiting the highest spread, spanning $250–$750. These distributions reflect the differing volatility and market dynamics of each stock, with META and NFLX experiencing higher fluctuations, likely influenced by company-specific events or broader market trends.
...
#| eval: true
#| label: tbl-pred-var
#| tbl-cap: "Random Sample of Predictive Variables for Modeling Adjusted Closing Price. Showcases a sample of the predictive variables derived from raw stock data, including lagged prices, rolling averages, simple moving averages (SMA), volatility, daily returns, and symbol encodings."
#| warning: false
...
```
@tbl-pred-var provides a snapshot of the engineered predictive variables used in the modeling process. **'Lag_1'** reflects the adjusted closing price from the prior day, serving as a baseline for trend predictions. Rolling averages and simple moving averages (e.g., 7-day, 20-day, and 50-day) smooth out short-term fluctuations and emphasize broader price trends. Volatility, calculated as the 20-day rolling standard deviation, quantifies market uncertainty, while daily returns measure the day-to-day percentage change in adjusted closing price. The inclusion of symbol encodings ensures the model distinguishes between stocks while analyzing aggregate trends. These variables were carefully selected to balance short-term variability with medium-term patterns, enabling robust and reliable predictions.
...
#| fig-cap: "Comparison of actual (blue) vs. predicted (red) adjusted closing stock prices for FAANG companies in November 2024. Evaluates the November model’s predictive performance across Apple (AAPL), Amazon (AMZN), Google (GOOGL), Meta (META), and Netflix (NFLX)."
#| warning: false
...
```
@fig-nov-actual-pred indicates strong predictive performance for AAPL and META, where predicted prices closely follow actual trends, indicating the model's effectiveness in capturing both upward and downward market movements. AMZN and GOOGL exhibit moderate alignment, with some deviations occurring during periods of higher volatility, suggesting that while the model captures general trends, it struggles with abrupt changes. In contrast, NFLX demonstrates notable discrepancies, especially in the second half of November, where the predicted values fail to reflect the sharp upward trend observed in actual prices. These results highlight the model's capacity to replicate stable patterns but expose its limitations in adapting to rapid price movements, particularly for stocks like NFLX that experience heightened volatility. 
...
```
@fig-nov-avg displays that the November model predicts that Netflix (NFLX) will have the highest average stock price at approximately $750, reflecting its anticipated dominance among FAANG stocks. Meta (META) follows with an average price just above $500, while Apple (AAPL), Amazon (AMZN), and Google (GOOGL) show significantly lower predicted averages, at around $225, $210, and $180, respectively. This result emphasizes the expected outperformance of Netflix and Meta, suggesting stronger market confidence in these two companies for November. Meanwhile, the comparatively lower averages for AAPL, AMZN, and GOOGL may indicate either more stable price trends or less aggressive growth potential modeled for these stocks during this period.
...
#| eval: true
#| label: fig-dec-pred
#| fig-cap: "Predicted stock prices for FAANG companies in December 2024. Illustrates the predicted stock prices for FAANG companies during December 2024, highlighting flat trends across all stocks and minimal variation over the month."
#| warning: false
...
```
@fig-dec-pred reveals flat stock price trends for all FAANG companies, with no significant fluctuations throughout the month. Netflix (NFLX) is projected to maintain the highest price, exceeding $875, while Google (GOOGL) consistently exhibits the lowest price, just above $200. Apple (AAPL), Amazon (AMZN), and Meta (META) display similar stability, with their prices clustered between $250 and $600. These flat trends suggest that the model struggles to capture potential market dynamics and daily volatilities, likely due to the absence of real-world influences such as macroeconomic shifts or investor sentiment in the input data. Further discussion on these limitations is provided in [Appendix -@sec-model-perform-limit].
...
```
@fig-dec-avg shows that Netflix (NFLX) is expected to outperform its peers with an average price nearing $825, followed by Meta (META) at approximately $600. Apple (AAPL), Amazon (AMZN), and Google (GOOGL) show significantly lower average predicted prices, clustering around $250, $200, and $200, respectively. These results suggest Netflix’s dominance in price performance among FAANG stocks, likely reflecting historical trends captured by the model. However, the flat trends and low variability in predictions suggest limitations in the model's capacity to incorporate dynamic market factors and real-world volatility.
...
```
@fig-nov-dec-pred displays contrasting trends - November predictions reflect the market's dynamic nature, with AAPL and META showing upward trends and AMZN and GOOGL demonstrating fluctuations. Conversely, December predictions exhibit flat trends across all FAANG companies, particularly for NFLX, which remains static throughout the month. This stark contrast underscores the model's inability to adapt to market changes and capture potential end-of-year volatilities, limiting its effectiveness for December forecasting.
...
```
@fig-nov-dec-avg compares the average predicted stock prices for FAANG companies in November (red) and December (teal) 2024, revealing varying dynamics across the companies. Netflix (NFLX) consistently leads with the highest prices, increasing significantly from November to December. Meta (META) follows a similar upward trend, albeit less pronounced. In contrast, Apple (AAPL), Amazon (AMZN), and Google (GOOGL) remain relatively stable, reflecting the model's ability to capture consistent trends for these stocks. Google's slight decrease between the months may suggest unique market dynamics requiring deeper investigation, particularly in the context of broader market sentiment and sector-specific factors. 
...
The predictive insights from this analysis hold significant value for investors, policymakers, and analysts, as FAANG stocks (Meta, Amazon, Apple, Netflix, Google) serve as key indicators of economic health and technological trends. Accurate stock price predictions enable informed decision-making in several areas:

1. **Investment Strategies**: Anticipating price trends helps investors optimize portfolios. For instance, the predicted December 2024 price stability may guide conservative allocations or sector diversification.

2. **Economic Forecasting**: FAANG performance offers insights into consumer behavior, innovation cycles, and macroeconomic conditions, aiding policymakers in evaluating sectoral resilience.

3. **Risk Management**: Understanding volatility supports effective hedging strategies during high-risk periods, such as earnings reports or economic announcements.

4. **Tech Sector Analysis**: Predictions inform strategic decisions for tech-driven businesses, highlighting market expectations for ad-reliant firms like Meta and Google or streaming services like Netflix.

5. **Behavioral Insights**: Stock price trends reflect investor sentiment and external factors, emphasizing the need to integrate qualitative drivers, like trust in technology, into market models.

These applications demonstrate the practical utility of FAANG stock predictions in navigating dynamic market environments and making data-driven decisions.

### US Election Effect
A critical component of stock market behavior is its sensitivity to major political events, such as U.S. presidential elections. These events often drive short-term volatility and influence investor sentiment, leading to measurable shifts in stock prices across sectors. Analyzing the performance of FAANG stocks before and after elections in 2016, 2020, and 2024 provides valuable insights into how market participants react to political outcomes and how these reactions shape price trajectories in the post-election period.
...
#| tbl-cap: "Stock prices of FAANG companies one trading day before and after the U.S. Presidential Elections in 2016, 2020, and 2024. Most companies show an upward price change after the elections, particularly in 2024, where all FAANG stocks experienced increases, with Netflix (NFLX) recording the largest absolute gain."
#| warning: false
...
## Model Justification - Further Details {#sec-model-justification}
...
#| fig-cap: "Feature importance for the November model. Lagged stock price (Lag_1) is the most significant predictor, followed by the 7-day rolling mean (Rolling_Mean_7). Other features, including daily returns, simple moving averages (sma_20, sma_50), volatility, and symbol encoding, contribute minimally to the model's predictions."
#| warning: false
#| cache: false
...
#| fig-cap: "Feature importance for the December model. Highlights Lag_1 as the most influential predictor, followed by the 7-day rolling mean (Rolling_Mean_7). Other features, including daily returns, simple moving averages (sma_20, sma_50), volatility, and symbol encoding, have minimal contributions to the model's predictive capability."
#| warning: false
#| cache: false
...
#| fig-cap: "Residuals vs. fitted values for the November model. Residuals are centered around zero, indicating no major bias, but increased variability at higher fitted values suggests reduced accuracy for extreme stock prices."
#| warning: false
...
## Model Performance and Limitations {#sec-model-perform-limit}
...
## Current Analysis: Surveys, Sampling, and Observational Data
...
## Idealized Methodology for FAANG Stock Prediction

Building on the performance and limitations of our analysis, we now propose a robust framework for predicting FAANG stock prices by integrating observational data with investor sentiment surveys. The approach addresses limitations such as missing values, external confounders, and market volatility, incorporating best practices to ensure accurate, actionable predictions. It is designed to operate within a budget of $100,000.

The link to our survey is as follows: "https://docs.google.com/forms/d/e/1FAIpQLScbhRrXNXA-70g6SplY8TcupSMQePFi3W6BbnwuAf1EQSrKJw/viewform?usp=sf_link"

### Target Population, Sampling Frame, and Sample

- **Target population**: Active retail investors who have traded FAANG stocks at least once in the past year. This group represents individuals most engaged with FAANG stock dynamics and whose sentiment provides actionable insights for predictive modeling.
  
- **Sampling frame**: A comprehensive list of active retail investors derived from brokerage datasets. The frame includes demographic details (e.g., age, income, gender), trading behavior (e.g., frequency of trades), and portfolio composition (e.g., FAANG stock exposure).

- **Sample**: A stratified sample of 1,000 respondents is selected to ensure proportional representation across key dimensions, including age (e.g., 18–34, 35–54, 55+), income level, portfolio size, and trading frequency. This stratification ensures diversity and reliability in capturing investor sentiment.


### Sampling Methodology & Justification

The sampling methodology employs stratified random sampling to ensure representation across diverse investor demographics. Respondents are stratified along the following dimensions:

1. **Demographics**: Age, gender, and income levels.  
2. **Portfolio Characteristics**: Size of portfolios and proportion allocated to FAANG stocks.  
3. **Trading Behavior**: Frequency of trades, distinguishing between high-frequency and occasional investors.

Stratification ensures the sample mirrors the diversity within the broader investor population. This method enhances generalizability while maintaining cost efficiency. With a sample size of 1,000, the methodology balances statistical rigor with practical feasibility.



### Survey Design

The survey is structured to capture critical insights into investor sentiment, preferences, and behaviors influencing FAANG stock price movements. It focuses on key dimensions such as market confidence, risk appetite, and reactions to external events like the 2024 U.S. Presidential Election. Designed with clarity and brevity, the survey ensures high-quality responses by minimizing respondent fatigue through concise, well-phrased questions. It incorporates a mix of quantitative and qualitative metrics, allowing respondents to express the intensity of their opinions on a scale-based format, providing a nuanced understanding of market sentiment.

Our survey design also accounts for inclusivity and accessibility by being optimized for desktop and mobile platforms. To ensure reliability, validation questions are included to identify inattentive responses, and demographic questions enable stratified analysis across investor types. By framing questions within realistic contexts, such as recent market events and hypothetical FAANG performance scenarios, the survey gathers actionable data that enhances the interpretability and predictive power of stock price models. This design bridges the gap between quantitative trends and investor psychology, offering a comprehensive tool for integrating sentiment into financial forecasting.

#### Cost and Feasibility

The methodology is designed to operate within a $100,000 budget:

- **Survey Development and Distribution**: $50,000 for platform costs and participant incentives.
- **Data Acquisition**: $30,000 for accessing brokerage datasets and sampling frames.  
- **Analysis and Reporting**: $20,000 for data integration, modeling, and reporting.


### Data Validation

To ensure reliable and representative survey data, we employ the following:

- **Weighting**: Adjust responses to reflect population proportions using the formula:  
$$
  \text{Weight} = \frac{\text{Population Proportion}}{\text{Sample Proportion}}
$$
  This corrects for over- or under-representation within the sample.  
- **Validation Questions**: Include items like “Select ‘Agree’ from the options below” to identify inattentive responses.  
- **Unique IDs**: Assign each respondent a unique ID to prevent duplicates, retaining only the first response.  
- **Real-Time Monitoring**: Track responses in real time to identify potential inconsistencies or fraudulent submissions.



### Integration with Observational Data

Survey responses will be integrated with historical stock price data by mapping survey-derived sentiment metrics, such as market confidence and risk appetite, to relevant time periods in the stock price dataset. For example, survey responses collected during November and December 2024 will be linked to corresponding stock price data, allowing the models to incorporate real-time investor sentiment alongside historical price trends. These sentiment metrics will be treated as additional predictive features in the modeling framework, enabling the machine learning models to capture the relationship between investor expectations and subsequent stock price movements. For instance, metrics like market confidence will be aligned with price trends to evaluate whether periods of high investor optimism correlate with upward price movements. Similarly, risk appetite data will help contextualize market volatility and identify patterns during periods of uncertainty.

By combining survey insights with lagged prices, rolling averages, and volatility measures from the historical data, the models will leverage both forward-looking sentiment and past market behavior to make more robust and actionable predictions. This integration bridges the gap between investor psychology and quantitative market analysis, enhancing the models' ability to forecast stock price dynamics during volatile or event-driven periods.

### Survey Demo

Full survey can be found at "https://docs.google.com/forms/d/e/1FAIpQLScbhRrXNXA-70g6SplY8TcupSMQePFi3W6BbnwuAf1EQSrKJw/viewform?usp=sf_link" and under `FAANG-Stock-Forecast/other/survey`

**Title: "Investor Sentiment Survey for FAANG Stock Predictions"**

#### Survey Introduction
We are conducting an important survey to understand investor opinions and predict FAANG stock price movements. Your participation will provide valuable insights into market sentiment and help improve stock price forecasting models.
...
#### Survey Questions:
1. **Market Confidence**: “Do you expect FAANG stock prices to increase, decrease, or remain stable over the next month?”  
...
2. **Risk Appetite**: “What is your current risk appetite for investing in technology stocks?”  
   - High  
   - Moderate  
   - Low  

3. **External Factors**: “To what extent have recent market events (e.g., 2024 U.S. Presidential Election, Federal Reserve decisions) influenced your investment decisions?”  
   - A great deal  
   - Somewhat  
   - Not at all  

4. **Sector Preference**: “Besides FAANG stocks, which sectors are you considering for your portfolio over the next quarter?”  
   - Technology (non-FAANG)  
   - Healthcare  
   - Energy  
   - Consumer Goods  
   - Other (please specify)  

5. **Economic Outlook**: “How do you perceive the broader economic climate for technology investments?”  
   - Strong growth  
   - Moderate growth  
   - Stagnation  
   - Decline  
